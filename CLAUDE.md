...
| `src/source/record/` | records in general, above any one record *format* (SPEC.md §Lenses). There is **one** record source — `source.rs` (`RecordSource<S>`: state, cache, search sweep, the lens state and its forwarders), `view.rs` (the `Source` impl), `rows.rs` (summary rows, expansion, outline, `--toc`), `tree.rs` (one record laid out as tree rows, over `jsonrow`'s grammar) — and what a record *format* supplies is `store.rs`'s `Store`: how many records the index has found, how to push it along inside a byte budget, and record `i` as bytes or as a value. Nothing here opens a file, names a format or mentions a line. The lens machinery under it is `plan.rs` (items: which records share a row, and the two-level row arithmetic — an item's own rows depend on the **width**, because a message is wrapped, so `set_width` re-lays every body without reclassifying anything), `rowmap.rs` (which rows an open record owns), `lensrow.rs` (the rows a lens paints, the row-to-record translation under them, and the body measurement), `body.rs` (the message under a summary row: its wrap, its six-row clip, and the `⋯ +N lines` row that says what the clip left out — a lens never hides anything), `ops.rs` (what `zR`, `zM`, `Tab`, `Y` and a fold id off the outline do to a plan), and the shared gutter and fold vocabulary: `marker`/`leaf`, `/4` for a record beside `plan::group_id`'s `g4` for a group, the two spellings that must not collide. `Records` is the trait `lensrow`/`ops` reach records through — `with_value` hands the record to a closure, so it is generic rather than `dyn`: a record can be tens of megabytes and lives behind a `RefCell`, and handing out a clone to read one field would undo the cache. A change all lenses need belongs here, not in a dialect |
