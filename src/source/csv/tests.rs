...
pub(super) fn src_from(body: &str) -> CsvSource {
    let mut s = CsvSource::from_bytes(body.as_bytes().to_vec(), None);
    s.set_width(80);
    s
}

pub(super) fn text(s: &mut CsvSource, row: usize) -> String {
    s.lines(row..row + 1).pop().map(|l| l.text()).unwrap_or_default()
}

pub(super) fn all(s: &mut CsvSource) -> Vec<String> {
    let n = s.len();
    s.lines(0..n).iter().map(|l| l.text()).collect()
}

pub(super) const SMALL: &str = "id,name,city\n1,alice,berlin\n2,bo,rome\n3,carolina,oslo\n";
