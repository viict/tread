...
//!
//! Split from `source.rs`, which holds the state and the reads, and from
//! `folds.rs`, which holds everything about folding — so all three stay under
//! the size limit. Written once for every record format: what a `.jsonl` and an
//! array inside a document disagree about is [`Store`] and nothing here. The
//! order of the sections below is the order of the trait's own
//! (`src/source/mod.rs`), and every method a record file has no honest answer
//! for says so rather than inventing one. A method whose body is one call into
//! `folds.rs` is a fold question and is documented there.
#![deny(unsafe_code)]
