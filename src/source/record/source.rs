...
/// The lens state, and the thin forwarders that reach it.
#[path = "lensstate.rs"]
mod lensstate;
...
use super::tree;
use super::{body, fold_id, leaf, lensrow, marker, ops, parts, Records};
...
pub struct RecordSource<S: Store> {
    ...
    /// The part rows of the one record the reader has open at the level between
    /// its headline and its JSON, keyed by width. Same discipline as
    /// `body_laid`: one record, rebuilt when the reader moves on.
    parts_laid: RefCell<Option<(usize, usize, parts::Laid)>>,
