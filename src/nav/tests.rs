...
fn link_kinds_are_classified() {
    ...
    // A file that is not markdown is followable now that the reader can show
    // any file (SPEC.md §Plain text). This used to assert `Target::Other`,
    // which was the refusal that section removes.
    assert!(matches!(
        resolve("assets/diagram.png", "/corpus"),
        Target::Doc { ref path, anchor: None } if path.ends_with("diagram.png")
    ));
